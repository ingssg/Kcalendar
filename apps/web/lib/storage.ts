...
export function addFoodEntries(date: string, newEntries: FoodEntry[]): DayRecord {
...
export function updateFoodEntryCalories(date: string, entryId: string, calories: number): DayRecord | null {
  const record = getDayRecord(date)
  if (!record) return null

  const entries = record.entries.map((e) =>
    e.id === entryId ? { ...e, calories } : e
  )
  const totalCalories = entries.reduce((sum, e) => sum + (e.calories ?? 0), 0)
